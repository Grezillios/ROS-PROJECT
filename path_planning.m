...
% Last Modified by GUIDE v2.5 14-May-2016 04:18:37
...
% varargin   command line arguments to path_planning (see VARARGIN)

% Choose default command line output for path_planning
...
% handles    structure with handles and user data (see GUIDATA)
global simpleMap
[path, addr] = uigetfile('*.jpg');
simpleMap = imread(strcat(addr,path));
simpleMap  = im2bw(simpleMap );
axes(handles.axes1) , imshow(simpleMap), title('Load Image')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global simpleMap map
number_cell = inputdlg('Enter the number of cel per Meter');
number_cell = str2num(cell2mat(number_cell));
map = robotics.BinaryOccupancyGrid(simpleMap, number_cell)

axes(handles.axes1) , show(map) , title('Image convert to Map')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global map mapInflated
% robotRadius = 0.2;
robotRadius = inputdlg('Enter the Robot Radius');
robotRadius = str2num(cell2mat(robotRadius));

mapInflated = copy(map);
inflate(mapInflated,robotRadius);

% Display inflated map
axes(handles.axes1) , show(mapInflated)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global mapInflated prm
prm = robotics.PRM

% Assign the inflated map to the PRM object
prm.Map = mapInflated;
axes(handles.axes1) ,show(prm)
...
startLocation = ginput(1) %[2 1];
endLocation = ginput(1)  %[12 10];

% set(Handling.findpath,'callback',{@findpath_callback,Handling})
...
v1= get(handles.edit1,'String');  % 50 
prm.NumNodes = str2num(v1);

v2= get(handles.edit2,'String');  %5
prm.NumNodes = str2num(v2);
